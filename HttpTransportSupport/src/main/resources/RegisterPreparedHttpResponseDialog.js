/*
    The RegisteredPreparedHttpResponseDialog takes a prepared response for editing, or if the argument is null it produces a new dialogue.
    PreparedHTTPResponse consist of a HTTP response to send, and some filters for when to send it.
*/

class RegisteredPreparedHttpResponseDialog{
    constructor(preparedStatementToView){
        this.preparedStatement = this.setDefaultsIfNullOrUseAsIs(preparedStatementToView);
        this.clientheaders = [];
        ...
        div1.appendChild(new HttpResponseDiv(this.preparedStatement.response).rootElement);
...
class HttpResponseDiv{
    constructor(httpResponse){
        if(httpResponse == null){
            ...
            httpResponse.delayInMs = 0;
            httpResponse.httpRequest = {};
            httpResponse.bodyFilePath = null;
            httpResponse.uri = null;
            httpResponse.bodyType = "direct";
        }
        ...
        var bodycontenttypedropdown = document.createElement('select');
        bodycontenttypedropdown.id = "bodycontenttypedropdown";

        var directOption = document.createElement('option');
        directOption.value = "direct";
        directOption.innerHTML = "Direct input";
        bodycontenttypedropdown.appendChild(directOption);

        var filePathOption = document.createElement('option');
        filePathOption.value = "file";
        filePathOption.innerHTML = "File read by server at runtime";
        bodycontenttypedropdown.appendChild(filePathOption);

        bodycontenttypedropdown.addEventListener('change', function(){
            this.bodyType = bodycontenttypedropdown.value;
        }.bind(this.response));
        responseBodyPanel.appendChild(bodycontenttypedropdown);
        ...
        responsebodycontent.innerHTML = httpResponse.body;
        ...
        responsePanel.appendChild(responseBodyPanel);

        var advancedHeading = document.createElement('h4');
        advancedHeading.style.cursor = "pointer";
        advancedHeading.style.marginTop = "25px";
        advancedHeading.onclick = "if(document.getElementById('advancedHttp').style.display == 'block'){document.getElementById('advancedHttp').style.display = 'none';this.innerHTML='Advanced &#709;';}else{document.getElementById('advancedHttp').style.display = 'block';this.innerHTML='Advanced &#708;';};";
        ...
        advancedPanel.innerHTML = '<table id="responsecodesection"><tr><td>Response code for registered response:</td><td><input id="responsecode" type="number" value=200></td></tr><tr><td>Server side response delay (ms):</td><td><input id="delay" type="number" value=0></td></tr></table>';
        advancedPanel.innerHTML += '<hr /><div id="responseheaders"><h4>Register response headers</h4><div id="headervalues"></div>';
        ...
        responsePanel.appendChild(advancedPanel);

        var filterPanel = document.createElement('div');
        filterPanel.classList.add('panel');
        filterPanel.id = "responsefilters";
        filterPanel.innerHTML = "<h3 class='tooltip'>Triggers for when to send this response<span class='tooltiptext'>All registered triggers for a prepared response need to be fullfilled for the registered response to be sent. If no trigger filter is applied the response will always be sent (if no other response are sent first).</span></h3><div id='registeredtriggers'></div>";
        ...
        triggerTypeCell.innerHTML += "<option value='NextResponse'>Next response</option>";
        triggerTypeCell.innerHTML += "<option value='OriginUrlFilter'>Origin URL</option>";
        triggerTypeCell.innerHTML += "<option value='HttpMethodVerbFilter'>HTTP method verb</option>";
        triggerTypeCell.innerHTML += "<option value='EndpointUrlFilter'>End-point path</option>";
        triggerTypeCell.innerHTML += "<option value='HeaderExistsFilter'>Header exist</option>";
        triggerTypeCell.innerHTML += "<option value='HeaderValueFilter'>Header value</option>";
        triggerTypeCell.innerHTML += "<option value='TimePeriodFilter'>Time period</option>";
        triggerTypeCell.innerHTML += "<option value='BodyRegexMatchFilter'>Body regular expression match</option>";
        triggerTypeCell.innerHTML += "<option value='BodyContainsMatchFilter'>Body contains</option>";
        triggerTypeCell.innerHTML += "</select><br />";
        triggerTypeRow.appendChild(triggerTypeCell);
        ...
        infoIcon.onclick = "document.getElementById('triggerTypeInfoOverlay').style.display = 'block';";
        ...
        field1Cell2div.innerHTML += '<button onclick="document.getElementById("';
        field1Cell2div.innerHTML += 'triggerTypeInfoOverlay';
        field1Cell2div.innerHTML += '").style.display = ';
        field1Cell2div.innerHTML += "'none';";
        field1Cell2div.innerHTML += '">Close</button>';
        ...
        this.rootElement = responsePanel;
    }
}
